...
export async function generateProjectedStatementAction({
  lang,
  temperature,
}: StatementOptions): Promise<ProjectedSkillsGrowthOutput> {
  const input = {
    ...
    futureAreasOfInterest:
      lang === 'es'
        ? 'Aunque mi corazón está en el backend, me entusiasma aprender sobre frontend, cloud, IA y arquitectura de software para construir soluciones completas.'
        : 'While my heart is in backend, I am excited to learn about frontend, cloud, AI, and software architecture to build complete solutions.',
    lang: lang,
